##See line 608 for new code snippet for Polynomial
##See line 109 for added Polynomial radio option
...
server <- function(input,output,session){
  ...
  uelw <- reactive({
    ...
    if (input$fn == "3") {
      t_plot <- t_plot + 
        lapply(s_df(), function(df) {
          # Create a data frame with a sequence of x values for smoother curves
          x_seq <- seq(min(df$rc), max(df$rc), length.out = 100)
          # Fit a polynomial model to the data
          fit <- lm(dg ~ poly(rc, 2, raw=TRUE), data=df)
          # Predict y values based on the fitted model
          y_pred <- predict(fit, newdata = data.frame(rc = x_seq))
          # Create a data frame for plotting
          df_poly <- data.frame(rc = x_seq, dg = y_pred)
          # Generate the plot layer
          geom_line(data = df_poly, aes(x = rc, y = dg), 
                    color = ifelse(input$mpath_c, epil_xrwm_c()[[which(sapply(s_df(), identical, df))]], input$c_col), 
                    size = as.numeric(input$csz), alpha = input$alpha)
        })
    }
